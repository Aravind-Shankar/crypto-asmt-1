/*
 *  File name:      savitr_timer.c
 *  Author(s):      Team ApBash [Aravind S (EE14B012), Aditya Pradeep (EE14B068)]
 *  Synopsis:       File to timestamp full ECB mode encryption/decyption using savitr.h functions.
 *  Execution:      Compile with savitr.c, or "make timer"
 *					Running the executable (encryption): savitr_timer e <inputFile> <keyFile> <outputFile>
 *					Running the executable (decryption): savitr_timer d <inputFile> <keyFile> <outputFile>
 *
...
// timestamp function, returns the value of the timestamp counter (clock cycles)
...
int main(int argc, char const *argv[])
{
	u32 keys[40];
	u8 *pt, *ct;
	u32 numBlocks, bytesRead, timeDiff;
